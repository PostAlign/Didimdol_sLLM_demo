...
test('output uploads reuse one GPU buffer with bounded staging and stop after cancelled reads', async () => {
  const originalUsage = globalThis.GPUBufferUsage;
  globalThis.GPUBufferUsage = { STORAGE: 128, COPY_DST: 8 };
  let created = 0, destroyed = 0, writes = 0, pending = 0, reads = 0;
  ...
  const device = {
    ...
    queue: { writeBuffer(buffer, offset, data) {
      assert.equal(pending, 0); assert.ok(data.length <= 2 * 2**20); writes++; pending++;
    }, async onSubmittedWorkDone() { pending = 0; } },
  };
  const ort = { Tensor: { fromGpuBuffer() { return { dispose() {} }; } } };
  const store = { async readRangeInto(file, offset, size) { assert.ok(size <= 2 * 2**20); reads++; } };
  let weights;
  try {
    ...
    assert.equal(created, 1); assert.equal(reads, 40); assert.equal(writes, 40);
    ...
    assert.equal(writes, 40, 'cancel during disk read must not enqueue another upload');
...
test('a disk error remains the primary failure while both GPU error scopes are drained', async () => {
  ...
  const weights = Object.assign(Object.create(StreamedWeights.prototype), {
    ...
    metrics: { outputReadBytes: 0 }, store: { async readRangeInto() { throw diskError; } },
