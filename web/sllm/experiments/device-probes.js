...
export async function residentProbe(manifest, checkpoint) {
  ...
  try {
    ...
    const scratch = new Float32Array(8 * 2**20 / 4).fill(1);
    for (const [index, init] of manifest.initializers.filter(t => t.location).entries()) {
      ...
      for (let offset = 0; offset < init.bytes; offset += scratch.byteLength) {
        await checkpoint({ stage: 'resident-write', initializerName: init.name, destinationOffset: offset, gpuWeightAllocated: allocated });
        device.queue.writeBuffer(buffer, offset, scratch, 0, Math.min(scratch.length, (init.bytes - offset) / 4));
    ...
    while (scopes) { scopes--; const error = await device.popErrorScope(); if (error) throw new Error(error.message); }
    return { gpuWeightAllocated: allocated, expectedBytes: manifest.totalExternalTensorBytes, allBytesUsed: true };
  } finally {
...
export async function runtimeProbe(mode, checkpoint, idleSeconds = 120) {
  ...
  try {
    ...
    loader = new SessionRangeLoader({ manifest, stagingMiB: 8, checkpoint });
    ...
    return { inferenceVerified: true, metrics: { ...loader.metrics }, storage: { ...store.metrics }, gpuLedger: { ...tracked.ledger }, idleSeconds };
  } finally {
    store.close();
...
self.onmessage = async ({ data }) => {
  ...
  try {
    const build = await fetch(new URL('../../vendor/build.json', import.meta.url)).then(r => r.json());
    journal.state.environment.build = build;
    let result;
    if (data.kind === 'resident') {
      ...
      result = await residentProbe(manifest, checkpoint);
    } else result = await runtimeProbe(data.mode || 'asyncify', checkpoint, data.idleSeconds ?? 120);
    result = { ...result, success: true, durationMs: performance.now() - start };
