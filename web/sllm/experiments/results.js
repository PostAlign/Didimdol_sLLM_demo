export const isResident = kind => ['resident', 'resident-opfs'].includes(kind);
export const isSimpleProbe = kind => isResident(kind) || kind === 'runtime';
...
export function executionSettings(kind, config = {}) {
  return { runtimeMode: isResident(kind) ? null : config.mode || 'asyncify',
    ...
    inputSource: kind === 'resident' ? 'synthetic' : kind === 'resident-opfs' ? 'opfs-cache' : null };
}
